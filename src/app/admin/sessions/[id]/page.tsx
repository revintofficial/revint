    ...
    <div className="space-y-5">
      <header className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-semibold text-[var(--leadac-text-1)]">
            Session{" "}
            <code className="text-[var(--leadac-300)] text-base">
        ...
        <div className="flex items-center gap-2">
          {replayUrl ? (
            <a
              ...
              className="inline-flex items-center gap-2 px-3 py-1.5 rounded-md text-sm bg-[var(--leadac-500)] text-black font-medium hover:bg-[var(--leadac-400)]"
            >
              <Play className="h-3.5 w-3.5" />
              Watch on PostHog
            </a>
          ) : (
            <span className="text-xs text-[var(--leadac-text-3)]">
      ...
      <section className="rounded-xl border border-[var(--leadac-border)] bg-[var(--leadac-card)] p-4 space-y-3">
      ...
      <section>
        ...
        <div className="rounded-xl border border-[var(--leadac-border)] bg-[var(--leadac-card)] divide-y divide-[var(--leadac-border)]">
          {pageViews.length === 0 && (
            <div className="px-4 py-3 text-sm text-[var(--leadac-text-3)]">
              No page views recorded.
            </div>
          )}
          {pageViews.map((pv) => (
            <div key={pv.id} className="px-4 py-3 flex items-center gap-4">
              <code className="flex-1 min-w-0 truncate text-sm text-[var(--leadac-text-1)]">
                {pv.path}
              </code>
              <div className="text-xs text-[var(--leadac-text-3)] tabular-nums w-20 text-right">
      ...
      <section>
        ...
        <div className="rounded-xl border border-[var(--leadac-border)] bg-[var(--leadac-card)] p-4">
          <SessionTimeline events={timelineEvents} />
        </div>
      </section>

      <details className="rounded-xl border border-[var(--leadac-border)] bg-[var(--leadac-card)] p-4">
        <summary className="text-sm text-[var(--leadac-text-2)] cursor-pointer">
          Raw JSON dump (debug)
        </summary>
        <pre className="mt-3 text-xs text-[var(--leadac-text-3)] overflow-auto max-h-96">
...
function Stat({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="rounded-lg border border-[var(--leadac-border)] bg-[var(--leadac-bg)] p-3">
      <div className="text-[10px] uppercase tracking-wider text-[var(--leadac-text-3)]">
        {label}
      </div>
      <div
        className={`mt-1 text-sm text-[var(--leadac-text-1)] ${mono ? "font-mono" : ""}`}
      >
        {value}
      </div>
    </div>
...
function Row({ k, v }: { k: string; v: React.ReactNode }) {
  return (
    <>
      <dt className="text-[var(--leadac-text-3)]">{k}</dt>
      <dd className="lg:col-span-2 text-[var(--leadac-text-1)] break-all">{v}</dd>
    </>
  );
}
