...
const colorMap: Record<string, string> = {
  red: "var(--revint-error)",
  orange: "var(--revint-warning)",
  yellow: "var(--revint-warning)",
  green: "var(--revint-success)",
};
      ...
      {campaigns.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {campaigns.map((campaign, index) => {
            const accentColor = colorMap[campaign.color] || "var(--revint-muted)";
            return (
              <Card
                key={campaign.id}
              ...
              >
                ...
                <CardContent>
                  <Link href={`/app/leads?${new URLSearchParams(campaign.filter).toString()}`}>
                    <Button variant="outline" className="w-full group-hover:border-(--revint-500)/30 group-hover:text-(--revint-500) transition-colors">
                      View Leads
              ...
              {packages.map((pkg) => (
                <div
                  ...
                  className={`rounded-2xl p-5 hover:shadow-md transition-shadow relative ${
                    pkg.isPopular
                      ? "border-2 border-(--revint-500)/30 bg-(--revint-500)/3"
                      : "border border-white/10 bg-white/5"
                  }`}
                >
                  {pkg.isPopular && (
                    <Badge className="absolute -top-2.5 right-4 bg-(--revint-500) text-white border-transparent">
                      <Star className="w-3 h-3 mr-1" />
                      Popular
                    </Badge>
                  )}
                  <h3 className="font-semibold text-lg text-white">{pkg.name}</h3>
                  <p className={`text-2xl font-bold mt-1 ${pkg.isPopular ? "text-(--revint-500)" : "text-white"}`}>
                    {pkg.priceLabel}
                  </p>
                    ...
                    <ul className="mt-4 space-y-2 text-sm text-white/60">
                      {pkg.features.map((item) => (
                        <li key={item} className="flex items-center gap-2">
                          {pkg.isPopular ? (
                            <Zap className="w-4 h-4 text-(--revint-500) shrink-0" />
                          ) : (
