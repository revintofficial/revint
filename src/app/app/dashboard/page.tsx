...
import { REVINT, getFunnelStepColor } from "@/lib/colors";
...
// Mono-primary KPI cards: every icon shares --revint-h, only the
...
const KPI_CONFIG = [
  { key: "totalLeads", label: "Total Leads", icon: Users, color: REVINT.primary400 },
  { key: "withWebsite", label: "Have Website", icon: Globe, color: REVINT.primary400 },
  { key: "averageScore", label: "Avg. Score", icon: TrendingUp, color: REVINT.primary300, suffix: "/100" },
  { key: "recentLeads", label: "This Week", icon: CalendarDays, color: REVINT.primary400 },
] as const;

function CustomTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ value: number }>; label?: string }) {
  if (!active || !payload?.length) return null;
  return (
    <div className="revint-card-glass px-4 py-3" style={{ boxShadow: "0 8px 24px rgba(0,0,0,0.35)" }}>
      <p className="text-xs font-medium" style={{ color: "var(--revint-text-2)" }}>{label}</p>
...
export default function DashboardPage() {
  ...
  if (!stats) {
      ...
      return (
        <div className="p-4 sm:p-6 md:p-8 lg:p-10">
          <Card className="p-12 text-center">
            <CircleX className="w-12 h-12 mx-auto mb-4" style={{ color: "var(--revint-muted)" }} />
            <h3 className="text-lg font-semibold text-white mb-2">
              We couldn&apos;t load your dashboard
            </h3>
            <p className="text-sm mb-4" style={{ color: "var(--revint-text-2)" }}>
    ...
    return (
      <div className="p-4 sm:p-6 md:p-8 lg:p-10">
        <Card className="p-12 text-center">
          <Search className="w-12 h-12 mx-auto mb-4" style={{ color: "var(--revint-muted)" }} />
  ...
  // keeping a single --revint-h. The eye reads stage progression without
    ...
    <div className="p-4 sm:p-6 md:p-8 lg:p-10 space-y-6">
      ...
      {nextAction && (
        <Card
          ...
          style={{
            background: "hsl(var(--revint-h) var(--revint-s) 50% / 0.04)",
            borderTop: "1px solid var(--revint-500)",
            border: "1px solid hsl(var(--revint-h) var(--revint-s) 50% / 0.18)",
          }}
        >
          <CardContent className="p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <div
                className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 revint-glow-cta"
                style={{ background: "var(--revint-500)" }}
              >
                <Sparkles className="w-5 h-5 text-white" />
              </div>
              <div>
                <p className="text-sm font-medium text-white">What to do next</p>
                <p className="text-sm" style={{ color: "var(--revint-text-2)" }}>{nextAction.message}</p>
              </div>
            </div>
            <Link href={nextAction.href}>
              <Button size="sm" className="revint-glow-cta">
                {nextAction.action}
            ...
            <Card key={kpi.key} className="glass-hover">
              <CardContent className="p-5">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-[13px] font-medium mb-2" style={{ color: "var(--revint-text-2)" }}>{kpi.label}</p>
                    <div className="text-2xl font-semibold text-white">
                      {value}
                      {"suffix" in kpi && kpi.suffix && (
                        <span className="text-base font-normal" style={{ color: "var(--revint-text-3)" }}>{kpi.suffix}</span>
                      )}
                    </div>
                    {kpi.key === "withWebsite" && (
                      <p className="text-xs mt-1" style={{ color: "var(--revint-text-3)" }}>{stats.withoutWebsite} without website</p>
                    )}
                    {kpi.key === "recentLeads" && (
                      <p className="text-xs mt-1" style={{ color: "var(--revint-text-3)" }}>new leads</p>
                    )}
                  </div>
                  <div
                    className="w-10 h-10 rounded-xl flex items-center justify-center"
                    style={{ backgroundColor: "hsl(var(--revint-h) var(--revint-s) 50% / 0.12)" }}
                  >
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between gap-2">
              ...
              <Link href="/app/leads?queue=today" className="text-xs hover:underline" style={{ color: "var(--revint-500)" }}>
                Open today&apos;s queue →
              </Link>
            </div>
            <p className="text-xs" style={{ color: "var(--revint-text-3)" }}>
          ...
          <CardContent>
            {stats.totalLeads > 0 ? (
                ...
                {funnelData.map((stage) => {
                  ...
                  return (
                    <div key={stage.name} className="flex items-center gap-3">
                      <span className="text-xs w-20 text-right shrink-0" style={{ color: "var(--revint-text-2)" }}>{stage.name}</span>
                      <div className="flex-1 h-7 rounded-lg overflow-hidden relative" style={{ backgroundColor: "var(--revint-hover)" }}>
                        <div
              ...
              <div className="py-12 text-center">
                <Bot className="w-10 h-10 mx-auto mb-3" style={{ color: "var(--revint-muted)" }} />
                <p className="text-sm" style={{ color: "var(--revint-text-3)" }}>No data yet</p>
              </div>
            )}
          </CardContent>
        </Card>
                    ...
                    >
                      <div
                        ...
                        style={{
                          ...
                          backgroundColor: REVINT.primary500,
                        }}
                      />
                      <span
            ...
            <div className="hidden md:block">
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={stats.boroughDistribution}>
                  ...
                  <XAxis
                    dataKey="borough"
                    tick={{ fontSize: 11, fill: REVINT.text2 }}
                    angle={-45}
                    textAnchor="end"
                    height={80}
                  ...
                  <Bar dataKey="count" fill={REVINT.primary500} radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
...
function ThroughputTile({
  ...
  return (
    <div
      ...
      style={{
        background: accent
          ? "hsl(var(--revint-h) var(--revint-s) 50% / 0.06)"
          : "var(--revint-hover)",
        borderColor: accent
          ? "hsl(var(--revint-h) var(--revint-s) 50% / 0.18)"
          : "var(--revint-border)",
      }}
    >
      <div className="flex items-center gap-2 mb-2">
        <div
          ...
          style={{ backgroundColor: "hsl(var(--revint-h) var(--revint-s) 50% / 0.12)" }}
        >
          <Icon className="w-4 h-4" style={{ color: accent ? REVINT.primary300 : REVINT.primary400 }} />
        </div>
        <span className="text-[12px]" style={{ color: "var(--revint-text-2)" }}>{label}</span>
      </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <p className="text-[13px] font-medium" style={{ color: "var(--revint-text-2)" }}>{title}</p>
        <span className="text-xs" style={{ color: "var(--revint-text-3)" }}>{total} total</span>
      </div>
      {total > 0 && (
        <div className="flex h-2 rounded-full overflow-hidden mb-3" style={{ backgroundColor: "var(--revint-hover)" }}>
          {items.map((s) => {
