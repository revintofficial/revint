...
/**
 * LegacyLeadDetailClient — the original 5-tab Lead Detail surface.
 *
 * Phase 0 of the Lead Detail v2 rebuild renamed the original page
 * default export to this component and moved it here, leaving the
 * server route at `src/app/app/leads/[id]/page.tsx` to pick between
 * legacy (this file) and `<LeadDetailV2Client/>` via a feature flag.
 *
 * The only edits versus the prior `page.tsx` body are:
 *   - default export renamed from `LeadDetailPage` to `LegacyLeadDetailClient`
 *   - the function now accepts `{ id: string }` directly instead of
 *     unwrapping a `Promise<{ id }>` via `use(params)` — the server
 *     wrapper awaits `params` and passes the resolved id down.
 *
 * No other behavior changes. `?v=1` must be byte-equivalent at the
 * DOM level to the pre-split page.
 */
...
export default function LegacyLeadDetailClient({ id }: { id: string }) {
