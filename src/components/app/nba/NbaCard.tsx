  ...
  useEffect(() => {
    ...
    const fetchOnce = async () => {
      try {
        ...
        setData(json);
        setLoading(false);
        // Auto-expand trace once the final NBA lands.
        if (json.final && !traceOpen) {
          setTraceOpen(true);
        }
        // Keep polling while waiting for final to arrive (cap ~3min).
        if (!json.final) {
