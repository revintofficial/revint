...
const PLAN_BADGE: Record<UserMenuProps["workspace"]["plan"], { label: string; color: string }> = {
  FREE: { label: "Free", color: "hsl(var(--revint-h) var(--revint-nts) 92% / 0.55)" },
  PRO: { label: "Pro Solo", color: "var(--revint-500)" },
  PRO_TEAM: { label: "Pro Team", color: "var(--revint-500)" },
  AGENCY: { label: "Agency", color: "var(--revint-400)" },
};
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          className={`flex items-center gap-2.5 w-full px-2 py-1.5 rounded-lg hover:bg-white/5 focus-visible:outline focus-visible:outline-2 focus-visible:outline-(--revint-500) ${collapsed ? "md:justify-center md:px-1" : ""}`}
